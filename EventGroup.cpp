...
EventGroup::~EventGroup() {
    // Registrations is non-owning so have to clear them before owned descendants die
    observers.clear();
...
bool EventGroup::transferChild(EventComponent* child, EventGroup* destination) {
    ...
    // update regustration and composure ownerships (transfer changes both collaborations)
    detach(child);
